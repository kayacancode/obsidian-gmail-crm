...
### Obsidian source correction (September 23)
Members can explicitly include their uploaded Obsidian snapshot. The all-people or prefixed selected-person scope applies; Granola-folder scopes exclude the vault. Projection includes people and recorded ties, without private note contexts or adjusted scores. Photos follow the existing profile opt-in, except for the owner's private view. Legacy salted vault identities remain namespaced; names alone never cause cross-source merging. Per-member source coverage reports absent, disabled, unavailable and limited data. Updated plugin publications retain isolates, support up to 10,000 contacts within the existing payload budget, and report included/excluded counts. Old installations must update and publish again to supply contacts previously dropped.

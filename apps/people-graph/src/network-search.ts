...
/** Below this a Jev ranking has said "unrelated, and probably no help": not a result. */
const JEV_SCORE_FLOOR=0.15;
...
/** Lowercased, punctuation-free words of any text: the one tokenisation both sides of a match use. */
function words(text:string):string[]{return text.toLowerCase().split(/[^\p{L}\p{N}]+/u).filter(Boolean);}

/** Lowercased, punctuation-free, stopword-free query terms; short noise words are dropped. */
export function queryTerms(query:string):string[]{
 const terms=words(query).filter(term=>term.length>1&&!STOPWORDS.has(term));
 return [...new Set(terms)];
}

/** Whole-word match on the normalised text, so "ai" never hits "email". */
function mentions(text:string,term:string):boolean{return words(text).includes(term);}
...
function countHits(person:SearchCandidate,terms:string[]):number{
 // One token set per text: a term counts once per text it appears in, as a whole word.
 const weighted=[person.name,person.company].map(text=>new Set(words(text??'')));
 const plain=[...person.evidence.flatMap(item=>[item.summary,item.title??'']),...person.contexts,...person.themes].map(text=>new Set(words(text)));
 let hits=0;
 for(const term of terms){
  for(const text of weighted)if(text.has(term))hits+=NAME_WEIGHT;
  for(const text of plain)if(text.has(term))hits+=1;
 }
 return hits;
}
...
/**
 * The ten best people, each with the evidence that explains why they are here. A candidate the
 * ranking scores at zero is not a result: the recency fillers keywordRank adds so the model has
 * something to judge must never be shown as an answer, and a query nobody matches returns none.
 * A Jev ranking also drops anything it rated barely above nothing.
 */
export function topResults(query:string,ranked:Ranked[],scores:number[],options:{checked?:boolean;limit?:number}={}):SearchResult[]{
 const terms=queryTerms(query);
 const floor=options.checked?JEV_SCORE_FLOOR:0;
 return ranked.map((person,index)=>({person,score:scores[index]??0}))
  .filter(({score:value})=>value>0&&value>=floor)
  .sort((a,b)=>b.score-a.score||recency(b.person.lastContact)-recency(a.person.lastContact))
  .slice(0,options.limit??RESULTS)
...
function reasonsFor(person:SearchCandidate,terms:string[]):SearchEvidence[]{
 ...
 const shared=ordered.filter(item=>terms.some(term=>mentions(item.summary,term)||mentions(item.title??'',term)));

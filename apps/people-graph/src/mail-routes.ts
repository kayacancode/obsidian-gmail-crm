...
export async function mailRoute(request:Request,env:MailEnv,owner:string){
 ...
 if(path==='/api/accounts/connect'){
  ...
  const payload=b64(new TextEncoder().encode(JSON.stringify({owner,nonce}))),state=payload+'.'+await opaque('oauth',payload,env.TOKEN_SECRET);const google=new URL('https://accounts.google.com/o/oauth2/v2/auth');Object.entries({client_id:env.GOOGLE_CLIENT_ID,redirect_uri:redirect,response_type:'code',scope:'openid email https://www.googleapis.com/auth/gmail.readonly https://www.googleapis.com/auth/contacts.readonly https://www.googleapis.com/auth/contacts.other.readonly https://www.googleapis.com/auth/calendar.events.readonly',access_type:'offline',prompt:'consent select_account',state,code_challenge:await digest(verifier),code_challenge_method:'S256'}).forEach(([k,v])=>google.searchParams.set(k,v));
  if(typeof body.email==='string'&&body.email.length<=320)google.searchParams.set('login_hint',body.email);
...
export async function personFeedbackRoute(request:Request,env:MailEnv,owner:string){
 if(request.method!=='POST')return json({error:'method_not_allowed'},405);
 if(request.headers.get('origin')!==new URL(request.url).origin)return json({error:'invalid_origin'},403);
 const raw=await request.text();if(raw.length>2000)return json({error:'invalid_request'},400);
 let body:any;try{body=JSON.parse(raw);}catch{return json({error:'invalid_request'},400);}
 if(!body||typeof body.personId!=='string'||!body.personId||body.personId.length>200||body.personId.includes('@')||!['boost','suppress','snooze','clear'].includes(body.action))return json({error:'invalid_request'},400);
 const stub=env.MAIL.getByName(owner);await stub.bindOwner(owner);
 try{return json(await stub.setPersonFeedback(body.personId,body.action));}catch(error){if(error instanceof Error&&error.message==='unknown_person')return json({error:'unknown_person',message:'This action is available for your synced web contacts.'},404);throw error;}
}

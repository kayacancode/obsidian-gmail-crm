 ...
 try{
  ...
  const w=await changeWorkspace(env.DB,id,w=>{
   ...
   if(method==='PUT'&&action==='contribution'){
    ...
    try{self.contribution={enabled:body.enabled,scope:normalizeShareScope(body.scope),level:normalizeShareLevel(body.level)};}catch{deny(400,'invalid_contribution');}return;
   }

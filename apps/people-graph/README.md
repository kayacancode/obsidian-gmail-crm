...
# Atlas activity and profiles

The main viewer includes Today, This week (Monday through today in the browser's time zone), 30 days, and All time. Hover or focus previews a window; click commits it. These lenses use the viewer's latest recorded direct contact, not a teammate's activity or a mention in notes. They do not claim exact conversation counts or reconstruct historic network snapshots.

The in-app daily people digest ranks recent direct contacts and private recorded context by date. Each entry states its evidence; “People on your mind” is a suggested reminder, not an inference about thoughts. Missing dates produce an honest empty state. No email digest or scheduled outbound message is enabled.

Profiles show recorded connection strength, a comparison to other measured direct contacts, recent context, and possible introduction routes. A connector's score is the viewer's relationship with that connector. Co-occurrence does not establish the connector's closeness to the target. Request intro opens an editable local draft with a Copy action; it sends nothing.

Existing Jev search results now highlight matching people in the graph and retain reasons in the selected profile. Query relevance and connection strength are labeled separately. Jev needs the existing `TYPESAFE_API_KEY` deployment secret; missing/unavailable Jev uses the existing explicitly labeled keyword fallback. Clearing the answer restores the exploration state. Existing Granola identity confirmation remains in the Granola connection UI.

The atlas now uses a deterministic spatial layout based on supported themes, company domains, and recorded edges. Layout positions stay stable while changing the camera or time window. Selecting a time window opens the activity timeline: individual synced email messages, visible Granola attendee records, and separately labelled private note mentions. Event selection highlights participants; person selection opens the profile. The snapshot carries up to 1,000 recent email messages and 500 recent meetings (bounded participant rows); this is not a complete account archive. Granola dates derived from note creation are labelled explicitly. Shared owners' message activity is not included.

The people sidebar separates Needs attention from Recently in touch. Attention suggestions use conservative rules over permitted private content summaries: possible questions and follow-ups, mentions across distinct notes, and contact gaps relative to available history. These are candidates for review, not verified unanswered messages or unresolved obligations; subject-only metadata never creates a content-based alert. Cards retain dated source excerpts and available provenance links. For synced web contacts, We connected (+10), Not interested (−10), Later (seven days), and Undo are stored per person on the server. Adjustments are applied once to the web People score and shown separately from measured connection strength. Suppression hides all attention suggestions for that person; a connection acknowledgement clears older suggestions, while new evidence can resurface them. Later preserves an existing score adjustment. Reviewed people provides Undo. Snapshot-only views retain browser-local suggestion controls. The local CLI reconnect feedback file remains separate; these web choices are not mirrored into that file. Recently in touch uses recorded direct contact and excludes note-only mentions.

### Google Calendar context

Accounts can grant `calendar.events.readonly` alongside Gmail and Contacts. Existing accounts must use **Enable Google Calendar** and approve the added permission. Enable Calendar API in the Google Cloud project. The importer pages primary-calendar events from one year before each sync through 30 days after it, up to 10,000 events per inbox. It refreshes on the inbox sync schedule, reconciles removed events on a completed refresh, and removes inaccessible Calendar data if authorization fails. Calendar failures do not stop Gmail imports. Disconnect removes that inbox's event copies.

Only event title, dates, RSVP state, attendee addresses, and Calendar source link are retained, privately in the owner's Durable Object. Calendar entries attach to people already present in the graph; they do not create speculative relationships or increase measured contact scores. The same iCalUID/start instance across inboxes is deduplicated. Declined attendees, resource calendars, and cancelled events are excluded. Upcoming shows dated invitations; mutually accepted invitations generate preparation suggestions and pause reconnect suggestions. Acceptance never proves attendance or automatically completes a promise. Calendar context is not exported through network sharing.

### One shared network (no Obsidian required)

Open **Accounts → Shared networks**. Create a workspace, then expand **Members, invitations & what I share**. Under **Invite teammate**, enter the teammate's Google sign-in email, choose **Create invitation link**, and copy the link to send yourself. Invitations expire after seven days and only the specified Google account can accept.

The recipient signs in and presses **Accept invitation**. They can immediately open the shared network; they do not need Obsidian, a vault, a CLI, or a connected inbox. Joining shares nothing. Each member separately selects **What I share**, chooses contacts and context level, and saves. **Add my inbox** connects another inbox owned by the same member; it does not invite a teammate. Gmail/Calendar and Granola connections are optional sources managed on the website.

Use the **Network** selector in Atlas to switch between My network and a workspace. Exact matching contact addresses merge into one workspace node on the server. Similar names or different addresses are not automatically merged. A profile lists each contributing member's measured relationship score and contact date; private feedback adjustments and personal attention queues are not shared. **Request intro** creates an editable draft, never sends a message. Workspace search, Wander topics and paths use only the contributed data. A recorded co-occurrence does not prove two people know one another.

Administrators can revoke invitations, remove members, transfer administration or delete the workspace. Members can stop contributing or leave. The workspace view checks membership on focus and every 30 seconds while open. Losing access clears it; an offline view is labelled stale. Existing pairwise Sharing remains available separately.

Limits: 20 members; each member contributes up to 500 contacts, 2,000 edges and 2,000 signals/200 themes. Workspace display retains up to 200 themes and 5,000 signals overall. Partial or capped contributions are labelled. Initials are used in shared views; contact photos and raw mailbox content are not exported. Calendar titles are excluded. Notes contribute only the selected sharing level, including bounded quoted statements only when explicitly chosen. Workspace membership gives no personal bot access.

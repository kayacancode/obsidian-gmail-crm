...
try{
 ...
 workspace.contribution={enabled:true,scope:{kind:'folders',ids:['private-folder']},level:'names'};await page.reload();await page.getByText('Members, invitations & what I share',{exact:true}).click();await page.getByLabel('Context to share').selectOption('themes');await page.getByRole('button',{name:'Save sharing choices'}).click();await page.getByText('Sharing choices saved.',{exact:true}).waitFor();assert.deepEqual(savedContribution.scope,{kind:'folders',ids:['private-folder']});

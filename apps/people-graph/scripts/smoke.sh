...
echo "--- static pages and modules"
for path in / /spatial.html /classic.html /accounts.html /scene.mjs /model.mjs /relationship-graph/graph.mjs /relationship-graph/relevance.mjs; do
  curl -sf "localhost:8788${path}" >/dev/null
done

echo "--- config exposes client id"
curl -sf localhost:8788/api/config | grep -q googleClientId

echo "--- unauthenticated APIs -> 401"
for path in /api/graph /api/token /api/accounts '/api/relevance?lens=my'; do
  code=$(curl -s -o /dev/null -w "%{http_code}" "localhost:8788${path}")
  [ "$code" = "401" ] || { echo "expected 401 for $path, got $code"; exit 1; }
done
...
echo "--- authenticated mutations reject missing Origin and Idempotency-Key before RPC"
EXPIRES=$(( $(date +%s) * 1000 + 86400000 ))
SESSION_PAYLOAD=$(printf '{"email":"%s","expires":%s}' "$EMAIL" "$EXPIRES" | openssl base64 -A | tr '+/' '-_' | tr -d '=')
SESSION_MAC=$( { printf 'session\0'; printf %s "$SESSION_PAYLOAD"; } | openssl dgst -sha256 -hmac "$TOKEN_SECRET" -binary | openssl base64 -A | tr '+/' '-_' | tr -d '=')
session=(-H "cookie: __Host-people-session=${SESSION_PAYLOAD}.${SESSION_MAC}")
for path in /api/retrieval/confirm /api/public-sources/confirm /api/themes/theme-one/feedback; do
  code=$(curl -s -o /dev/null -w "%{http_code}" "${json[@]}" "${session[@]}" -d '{}' "localhost:8788${path}")
  [ "$code" = "403" ] || { echo "expected missing-Origin 403 for $path, got $code"; exit 1; }
  code=$(curl -s -o /dev/null -w "%{http_code}" "${json[@]}" "${session[@]}" -H "origin: http://localhost:8788" -d '{}' "localhost:8788${path}")
  [ "$code" = "400" ] || { echo "expected missing-idempotency 400 for $path, got $code"; exit 1; }
done

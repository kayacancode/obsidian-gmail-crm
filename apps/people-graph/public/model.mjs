...
export function normalize(input){
 ...
 const ids=new Set();const nodes=input.nodes.map(n=>{if(!n||typeof n.id!=='string'||!n.id||ids.has(n.id))throw Error('Duplicate or missing person identity in graph.');ids.add(n.id);return {id:n.id,name:text(n.name,200)||'Unnamed person',company:text(n.company,200),companySource:text(n.companySource,40),role:text(n.role,200),photoUrl:safePhoto(n.photoUrl),strength:typeof n.strength==='number'&&Number.isFinite(n.strength)?Math.max(0,Math.min(100,n.strength)):null,lastContact:text(n.lastContact,40),quadrant:text(n.quadrant,30)};});
 ...
 return {nodes,edges,byId,ties,source:text(input.source,40),scoreModel:text(input.scoreModel,100),note:text(input.note,500),pushedAt:text(input.pushedAt,40)};
}
...
export function search(g,query){
 ...
 if(query.trim()&&!words.length){const q=query.trim().toLowerCase();return g.nodes.flatMap(node=>{const hits=[['Name',node.name],[node.companySource==='email_domain'?'Email domain':'Company',node.company],['Role',node.role]].filter(([,v])=>v.toLowerCase()===q);return hits.length?[{node,score:5,coverage:1,reasons:hits.map(([k,v])=>k+': '+v),missing:[]}]:[];});}
 return g.nodes.map(node=>{
  const fields=[['Name',node.name],[node.companySource==='email_domain'?'Email domain':'Company',node.company],['Role',node.role],...g.ties.get(node.id).flatMap(e=>e.contexts.map(c=>['Connection context',c]))];

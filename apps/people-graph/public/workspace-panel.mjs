...
export function createWorkspacePanel(root,{request,onUnauthorized}={}){
 ...
 async function details(root,w){
  ...
  button(sharing,'Save sharing choices',async()=>{if(selected.size>200){status.textContent='Choose up to 200 people, or use All my contacts.';return;}await call(base+'/contribution',{shareProfiles:profiles.checked,enabled:mode.value!=='none',scope:mode.value==='people'?{kind:'people',personIds:[...selected]}:mode.value==='folders'?w.contribution.scope:{kind:'all'},level:level.value},'PUT');await refresh();status.textContent='Sharing choices saved.';});root.append(sharing);

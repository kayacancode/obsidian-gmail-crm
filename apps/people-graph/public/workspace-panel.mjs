...
export function createWorkspacePanel(root,{request,onUnauthorized}={}){
 ...
 async function details(root,w){
  ...
  const vaultLabel=el('label'),vault=el('input');vault.type='checkbox';vault.checked=w.contribution.includeObsidian===true;vaultLabel.append(vault,document.createTextNode('Include my uploaded Obsidian people'));sharing.append(vaultLabel,el('p','Uses your latest published vault snapshot. Private notes and adjusted scores are excluded. Older snapshots cannot be merged with Gmail identities reliably; they remain separate records. Granola folder selections exclude Obsidian.'));
  const profileLabel=el('label'),profiles=el('input');profiles.type='checkbox';profiles.checked=w.contribution.shareProfiles===true;profileLabel.append(profiles,document.createTextNode('Share contact display names and photos'));sharing.append(profileLabel,el('p','Include the names (including email handles) and available Google contact photos you see in your own network. Applies only to the people selected below.'));
  ...
  async function showPeople(){choices.replaceChildren();if(mode.value!=='people')return;
   if(!people){const result=await call('/api/graph?source=web',null,'GET');if(g!==generation||!root.isConnected)return;people=(result.graph?.nodes||[]).filter(p=>p.directRelationship!==false);if(vault.checked){const pushed=await call('/api/graph?source=obsidian',null,'GET');if(g!==generation||!root.isConnected)return;people.push(...(pushed.graph?.nodes||[]).filter(p=>p.type!=='company'&&p.directRelationship!==false&&!p.via?.length).map(p=>({...p,id:'obsidian:'+p.id,name:p.name+' · Obsidian'})));}}
  ...
  vault.onchange=()=>{people=null;void run(showPeople);};
  ...
  button(sharing,'Save sharing choices',async()=>{if(selected.size>200){status.textContent='Choose up to 200 people, or use All my contacts.';return;}await call(base+'/contribution',{includeObsidian:vault.checked,shareProfiles:profiles.checked,enabled:mode.value!=='none',scope:mode.value==='people'?{kind:'people',personIds:[...selected]}:mode.value==='folders'?w.contribution.scope:{kind:'all'},level:level.value},'PUT');await refresh();status.textContent='Sharing choices saved.';});root.append(sharing);

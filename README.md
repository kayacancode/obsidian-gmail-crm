...
Run **Sync contacts** to begin retaining dated email metadata. Run **Sync calendar meeting data** to import up to one year of mutually accepted events. Run **Update staleness scores** to refresh the relationship edges used by paths/graph. Use **Refresh** in the workspace to reread notes and the saved contact index, including CLI changes.
